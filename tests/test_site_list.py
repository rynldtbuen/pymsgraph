...
from pymsgraph.models.site.list import FieldValueSet

if TYPE_CHECKING:
    from tests.conftest import MakeClient


def _make_fvs(data: dict | None = None) -> FieldValueSet:
    return FieldValueSet.from_graph(
        data=data or {},
        client=None,
        path="/sites/s/lists/l/items/i/fields",
    )


def test_fieldvalueset_serialize_empty_when_no_dirty():
    fvs = _make_fvs({"Title": "Old"})

    assert fvs._dirty == set()
    assert fvs.serialize() == {}


def test_fieldvalueset_serialize_returns_only_dirty_keys():
    fvs = _make_fvs({"Title": "Old", "Number": 1})

    fvs["Title"] = "New"

    assert fvs.serialize() == {"Title": "New"}
    # Untouched key not included
    assert "Number" not in fvs.serialize()


def test_fieldvalueset_serialize_no_dirty_when_value_unchanged():
    fvs = _make_fvs({"Title": "Same"})

    fvs["Title"] = "Same"  # __setitem__ guards against no-op writes

    assert fvs._dirty == set()
    assert fvs.serialize() == {}


def test_fieldvalueset_serialize_skips_keys_missing_from_graph_data():
    fvs = _make_fvs({"Title": "Old"})
    # Simulate a stray dirty marker without a matching value
    fvs._dirty.add("Ghost")
...
def test_fieldvalueset_serialize_includes_new_keys():
    fvs = _make_fvs({"Title": "Old"})

    fvs["NewCol"] = "v"

    assert fvs.serialize() == {"NewCol": "v"}
