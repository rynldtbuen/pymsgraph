...
def test_client_sites_lists_roundtrip(make_client: "MakeClient"):
    seen = []
    ...
    site = c.sites.get(id="site-123")
    lists = list(site.lists)  # triggers iteration/fetch
...
def test_list_items_property(make_client: "MakeClient"):
    seen = []
    ...
    c, _ = make_client(handler)
    site = c.sites.get(id="s123")
    lst = next(iter(site.lists))
...
def test_list_queryset_filter(make_client: "MakeClient"):
    captured = []
...
def test_list_items_queryset_get_and_iter(make_client: "MakeClient"):
    seen = []
...
def test_fieldvalueset_save(make_client: "MakeClient"):
    seen = []
...
def test_list_items_create(make_client: "MakeClient"):
    seen = []
    ...
    item = lst.items.create(fields={"Title": "Created", "Number": 10})
...
def test_list_item_delete_requires_force(make_client: "MakeClient"):
    def handler(request: httpx.Request) -> httpx.Response:
        if request.method == "GET":
            if request.url.path == "/v1.0/sites/s123":
                return httpx.Response(200, json={"id": "s123", "displayName": "Site"})
    ...
    # deleting without force should raise
    import pytest
