...
class WorkbookWorksheet(ReadOnlyModel):
    """
    ...
    def __repr__(self) -> str:
        return f"<WorkbookWorksheet: {self.name or self.id}>"
    ...
    async def insert_values(
        ...
        *,
        ...
        """
        Insert values into a worksheet range.

        Args:
            address:
                A1-style range address (for example `A1:B3`).
            values:
                Two-dimensional list payload for the target range.
            workbook_session_id:
                Optional workbook session id sent as `workbook-session-id`.

        Returns:
            dict[str, Any]:
                Graph range response payload.

        Raises:
            ValueError:
                If `address` is empty or `values` is not a 2D list.

        Example:
            ```python
            workbook = await item.workbook.get()
            sheet = await workbook.get_sheet(name="Sheet1")
            await sheet.insert_values(
                "A1:B2",
        ...
        if not addr:
            raise ValueError(
                f"{type(self).__name__} insert_values method requires an address."
            )
        if not isinstance(values, list) or any(not isinstance(r, list) for r in values):
            raise ValueError(
                f"{type(self).__name__} insert_values method requires values as list[list[Any]]."
            )

        escaped_address = addr.replace("'", "''")
        path = f"{self.path}/range(address='{escaped_address}')"
        body = {"values": values}
        headers: dict[str, str] | None = None
        if workbook_session_id:
            headers = {"workbook-session-id": workbook_session_id}

        data = await self._client.patch(path, body=body, headers=headers)
    ...
    async def get_values(
        ...
        *,
        ...
        Returns:
            list[list[Any]]:
                Range values in row-major order. Returns an empty list when the
                response has no `values` payload.
        ...
        Example:
            ...
            workbook = await item.workbook.get()
            sheet = await workbook.get_sheet(name="Sheet1")
            values = await sheet.get_values("A1:B2")
            ```
        """
        addr = (address or "").strip()
        if not addr:
            raise ValueError(
                f"{type(self).__name__} get_values method requires an address."
            )
    ...
    async def get(self) -> "WorkbookWorksheet":
        """
...
class Workbook(ReadOnlyModel, PropertyModel):
    ...
    def by_sheet(self, id_or_name: str) -> "WorkbookWorksheet":
        """
        ...
        if not value:
            raise ValueError(f"{type(self).__name__} sheet method requires an id/name.")
